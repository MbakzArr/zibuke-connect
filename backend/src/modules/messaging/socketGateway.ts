...
// Small in-memory cache of user id -> display name, so typing events don't hit
...
async function getUserName(userId: string): Promise<string> {
  ...
  const name = r.rows[0]?.full_name || 'Someone';
...
export function attachSocketServer(httpServer: http.Server, allowedOrigins: string[]) {
  ...
  io.on('connection', async (socket: AuthedSocket) => {
    ...
    // Include the typer's name so the UI can say "Thabo is typing".
    socket.on('typing:start', async (channelId: string) => {
      socket.to(`channel:${channelId}`).emit('typing:update', {
        channelId,
        userId: user.userId,
        name: await getUserName(user.userId),
        typing: true,
      });
    });
